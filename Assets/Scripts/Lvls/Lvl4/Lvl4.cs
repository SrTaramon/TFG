...
public class Lvl4 : MonoBehaviour
{
    ...
    public void updateScore(int min, int sec, int points, int errors){
        ...
        //mitja test1 1:35min
        if (!gameOver){
            if ((min < 1 || (min <= 1 && sec <= 25)) && (counter == 4 || counter == 5)) { //3 estrella
    ...
    private void saveScore(int estrelles, int temps){
        if (estrelles != 0){
            ...
            if (estrelles > SaveData.current.estrelles4){
                SaveData.current.estrelles4 = estrelles;
                SaveData.current.new4 = true;
            }
